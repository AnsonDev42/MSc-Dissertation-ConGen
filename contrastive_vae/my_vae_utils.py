...
class VAEDecoder(nn.Module):
    ...
    def __init__(self, latent_dim, intermediate_dim, z_shape, output_dim, filters, kernel_size, nlayers, bias=True):
        ...
        self.z_shape = [1, 128, 16, 16, 16]  # [1, 128, 40, 48, 40] for 160*192*160
        self.filters = filters
        self.kernel_size = kernel_size
        self.linear1 = nn.Linear(latent_dim, intermediate_dim, bias=bias)
    ...
    def forward(self, x):
        ...
        print(f"decoder: shape of x is {x.shape}")
        x = self.linear1(x)
        # x = self.bn1(x)
        x = F.relu(x)
        x = self.linear2(x)
...
class ContrastiveVAE(nn.Module):
    ...
    def forward(self, tg_inputs, bg_inputs):
        ...
        return tg_outputs, bg_outputs, \
            ...
            tc_loss, discriminator_loss, tg_z, bg_z, q_score, q_bar_score, tg_z, tg_s
        # else:
...
class VAE(nn.Module):
    def __init__(self, input_shape=(1, 160, 192, 160), intermediate_dim=128, latent_dim=32, beta=1, disentangle=True,
                 gamma=1, bias=True, batch_size=64, device=None):
        ...
        filters = 32  # 32
        nlayers = 2
        self.z_encoder = VAEEncoder(input_shape, intermediate_dim, latent_dim, filters, kernel_size, bias=bias)
        if input_shape == (1, 160, 192, 160):
            self.z_shape = [128, 40, 48, 40]
        elif input_shape == (1, 64, 64, 64):
            self.z_shape = [128, 16, 16, 16]
        else:
            res = self.z_encoder(torch.randn(*input_shape))
            self.z_shape = res[3]
            print(f"z_shape set to unverified {self.z_shape}")
            raise NotImplementedError("for now encoder is using hardcoded product of z_shape")
        self.decoder = VAEDecoder(latent_dim, intermediate_dim, self.z_shape, output_dim=input_shape,
                                  filters=filters,
                                  kernel_size=kernel_size, nlayers=nlayers, bias=bias)
        self.disentangle = disentangle
        self.beta = beta
    ...
    def forward(self, tg_inputs, ):
        tg_z_mean, tg_z_log_var, tg_z, shape_z = self.z_encoder(tg_inputs)
        # print("tg_z shape", tg_z.shape)
        # tg_s_mean, tg_s_log_var, tg_s, shape_s = self.s_encoder(tg_inputs)
        # bg_z_mean, bg_z_log_var, bg_z, _ = self.z_encoder(
        #     bg_inputs)  # s->z:conflict with example code but match with paper
        print(f"input of decoders: {tg_z.shape}")
        # tg_outputs = self.decoder(torch.cat((tg_z, tg_s), dim=-1))  # line 196 in vae_utils.py
        tg_outputs = self.decoder(tg_z)  # line 196 in vae_utils.py
        # bg_outputs = self.decoder(torch.cat((bg_z, torch.zeros_like(tg_z)), dim=-1))
        # fg_outputs = self.decoder(torch.cat((tg_z, torch.zeros_like(tg_z)), dim=-1))

        # if self.disentangle:
        batch_size = tg_inputs.size(0)
        return tg_outputs, tg_outputs, \
            tg_z_mean, tg_z_log_var, \
            tg_z


def vae_loss(original_dim,
             tg_inputs, bg_inputs, tg_outputs, bg_outputs,
             tg_z_mean, tg_z_log_var,
             beta=1.0, disentangle=True, gamma=1.0, recon_alpha=1.0):
    # Reconstruction Loss
    ...
    reconstruction_loss = recon_alpha * (tg_reconstruction_loss)
    ...
    if disentangle:
        ...
        return (reconstruction_loss.mean(), beta * kl_loss.mean(), beta * kl_loss.mean()
                , beta * kl_loss.mean()
                )  # discriminator_loss
        # print each loss
...
def cvae_loss(original_dim,
              ...
              tg_s_mean, tg_s_log_var,
              bg_z_mean, bg_z_log_var,
              beta=1.0, disentangle=True, gamma=1.0, tc_loss=None, discriminator_loss=None, recon_alpha=1.0):
    # Reconstruction Loss
    reconstruction_loss = nn.MSELoss(reduction="none")
    if tg_outputs.is_cuda:
        # print('tg_outputs is cuda, move outputs to device')
    ...
    reconstruction_loss = recon_alpha * (tg_reconstruction_loss + bg_reconstruction_loss)
    reconstruction_loss /= (original_dim[1] * original_dim[2] * original_dim[3])
    # * (
    # original_dim[1] * original_dim[2] * original_dim[3])  # 0.5*
    # \
...
def vae_loss(original_dim,
             tg_inputs, bg_inputs, tg_outputs, bg_outputs,
             tg_z_mean, tg_z_log_var,
             beta=1.0, disentangle=True, recon_alpha=1.0):
    # Reconstruction Loss
    reconstruction_loss = nn.MSELoss(reduction="none")
    if tg_outputs.is_cuda:
        # print('tg_outputs is cuda, move outputs to device')
    ...
    if disentangle:
        ...
        #     f'cvae_loss {reconstruction_loss.mean() + beta * kl_loss.mean() + discriminator_loss.mean() + gamma * tc_loss.mean()}'
        #     f' | reconstruction_loss {reconstruction_loss.mean()} | beta* kl_loss {beta * kl_loss.mean()} | gamma * tc_loss {gamma * tc_loss.mean()} | discriminator_loss {discriminator_loss.mean()}')
        return (reconstruction_loss.mean(), beta * kl_loss.mean()
                )  # discriminator_loss
        # print each loss
...
if __name__ == '__main__':
    ...
    tg_outputs, bg_outputs, tg_z_mean, tg_z_log_var, tg_z = model(tg_inputs,
                                                                  )
    ...
    tg_z_total.append(tg_z.detach().cpu().reshape(-1, 32))
    tg_z_mean_total.append(tg_z_mean.detach().cpu().reshape(-1, 32))
    ...
    print(f"tg{tg_z_mean_total.shape}")
    # print(f"bg_z_mean_total shape {bg_z_mean_total.shape}")
    ss = plot_32d_latent_space(tg_z_mean_total=tg_z_mean_total, bg_z_mean_total=tg_z_mean_total,
                               tg_label=tg_labels_total, bg_label=tg_labels_total,
                               plot=True,
                               name='test_img_32d', VAE=True)
    ...
    recon, beta_kl = vae_loss(input_dim,
                              ...
                              disentangle=True, recon_alpha=1.0)
